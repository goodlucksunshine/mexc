 ...
 * @desc ETH市场交易队列消费者
 */
public class EthMarketTradeConsumer implements ChannelAwareMessageListener {
    ...
    public void onMessage(Message message, Channel channel) throws IOException {
       ...
       /* String key = "EthMarketTradeConsumer";
        try {
            RedisUtil.lock(key, 500);*/
        MDC.put("exeNo", TrackUtil.generateChannelFlowNo("EMT"));
        ...
        if (message != null) {
            synchronized (this) {
                try {
                    logger.info(LogUtil.msg("EthMarketTradeConsumer:process", "收到ETH交易请求:" + JSON.toJSONString(mqMsgVo)));
                    ...
                    logger.info(LogUtil.msg("EthMarketTradeConsumer:process", "撮合处理成功：" + entrustOrder.getTradeNo()));
                    ...
                    logger.error(LogUtil.msg("EthMarketTradeConsumer:process", "ETH交易异常,放入队列重新交易:" + JSON.toJSONString(mqMsgVo)), e);
                }
            }
        }
       /* }catch (Exception e) {

...
public class BtcdClientImpl implements BtcdClient {
    ...
    public List<Payment> listTransactions(String account, long count, long offset)
            throws BitcoindException, CommunicationException {
        List<Object> params = CollectionUtils.asList(account, count, offset);
        String paymentsJson = rpcClient.execute(Commands.LIST_TRANSACTIONS.getName(), params);
        List<Payment> payments = rpcClient.getMapper().mapToList(paymentsJson, Payment.class);
    ...
    public Boolean checkAddress(String address) throws BitcoindException,
            CommunicationException {
        String addressInfoJson = rpcClient.execute(Commands.VALIDATE_ADDRESS.getName(), address);
        if (StringUtil.isEmpty(addressInfoJson)) {
            return false;
        }
        Map addressInfo = null;
        try {
            addressInfo = JSON.parseObject(addressInfoJson, Map.class);
        } catch (Exception e) {
            LOG.error("检查地址异常:", e);
            return false;
        }
        if (addressInfo.get("isvalid") != null && (boolean) addressInfo.get("isvalid")) {
            return true;
        } else {
            return false;
        }

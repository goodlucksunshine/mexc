...
Datafeeds.UDFCompatibleDatafeed.prototype.getBars = function(symbolInfo, resolution, rangeStartDate, rangeEndDate, onDataCallback, onErrorCallback) {
	...
	//console.log("resolution:"+resolution+" from:"+rangeStartDate+" to:"+rangeEndDate);
	var agoTime = rangeStartDate;

	if(resolution=='1') {
		agoTime = rangeEndDate - 60 * 60 * 24;//查一天历史记录
	}else if(resolution=='5') {
		agoTime = rangeEndDate - 60 * 60 * 24 * 2;//查3天历史记录
	}else if(resolution=='15') {
		agoTime = rangeEndDate - 60 * 60 * 24 * 3;//5天
	}else if(resolution=='30') {
        agoTime = rangeEndDate - 60 * 60 * 24 * 5;
    }else if(resolution=='1h') {
		agoTime = rangeEndDate - 60 * 60 * 24 * 7;
	}else if(resolution=='D'||resolution=='1D') {
		agoTime = rangeEndDate - 60 * 60 * 24 * 30;
	}else if(resolution=='M'||resolution=='1M') {
        agoTime = rangeEndDate - 60 * 60 * 24 * 30 * 12;
    }

	if(rangeStartDate <= agoTime) {
		rangeStartDate = agoTime;
	}

	this._send(this._datafeedURL + this._historyURL, {
		//symbol: symbolInfo.ticker.toUpperCase(),
        symbol: marketName+"/"+vcoinNameEn,
		resolution: resolution,
		from: rangeStartDate,
		to: rangeEndDate,
		marketId:marketId,

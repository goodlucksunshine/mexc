...
/**
 * redis是单线程服务器。它的设计并不能从多核cpu中受益。
 * 可以安装多个redis实例来提高多核cpu的利用率。
 * 拿redis更多线程存储服务器做对比是不公平的。
...
public class RedisUtil {
    ...
    private static String redis_host= PropertyPlaceholderConfigurer.getProperty("REDIS_HOST");
    ...
    private static final int LOCKKEY_EXPIRE_TIME = 5;
    ...
    static {
        try {
            ...
            pool = new JedisPool(config, redis_host, PORT, TIMEOUT);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public static String set(String key, String value) {
        ...
        try {
            jedis = pool.getResource();
            jedisLock = new JedisLock();
            if (jedisLock.lock(key, 500, jedis)) {
    ...
    public static Long del(String... keys) {
        ...
        try {
            ...
            for (int i = 0; i < array.length; i++) {
                if (!jedisLock.lock(array[i], 0, jedis)) {
                    flag = false;
                }
            }
            if (flag) {
            ...
            for (int i = 0; i < array.length; i++) {
                jedisLock.unLock(array[i], jedis);
            }
            returnResource(pool, jedis);
        }
        return 0L;
    ...
    public static String rpoplpush(String srckey, String dstkey) {
        ...
        try {
            jedis = pool.getResource();
            jedisLock = new JedisLock();
            if (jedisLock.lock(srckey, 5000, jedis)) {
                res = jedis.rpoplpush(srckey, dstkey);
            }
        } catch (Exception e) {
            logger.error(e.getMessage());
        } finally {
            jedisLock.unLock(srckey, jedis);
            returnResource(pool, jedis);
        }
        return res;
    }
    ...
    public static Long sdiffstore(String dstkey, String... keys) {
        ...
        try {
            jedis = pool.getResource();
            jedisLock = new JedisLock();
            if (jedisLock.lock(dstkey, 500, jedis)) {
    ...
    public static Long sinterstore(String dstkey, String... keys) {
        Jedis jedis = null;
        Long res = null;
        JedisLock jedusLock = null;
        try {

            jedis = pool.getResource();
            jedusLock = new JedisLock();
            if (jedusLock.lock(dstkey, 500, jedis)) {
    ...
    public static Set<String> zrangeByScore(String key, BigDecimal max, BigDecimal min) {
        Jedis jedis = null;
        Set<String> res = null;
        try {
            jedis = pool.getResource();
            res = jedis.zrevrangeByScore(key, max.toPlainString(), min.toPlainString());
        } catch (Exception e) {
